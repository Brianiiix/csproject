            ...
            for(int k = 0; k < map.at(i).at(0).size(); k++){
                ...
                if(map[i][j][k].type == 'P'){
                    ...
                    for(int n = 0; n < net_num; n++){
                        // ¬a => ¬b ∧ ¬c ∧ ¬d ∧ ¬e 
                        ls.push_back(to_string(map[i][j][k].var_id[n]) + ' ' + to_string(map[i][j-1][k].var_id[n]) + " 0");
                        ls.push_back(to_string(map[i][j][k].var_id[n]) + ' ' + to_string(map[i][j+1][k].var_id[n]) + " 0");
                        ls.push_back(to_string(map[i][j][k].var_id[n]) + ' ' + to_string(map[i][j][k-1].var_id[n]) + " 0");
                        ls.push_back(to_string(map[i][j][k].var_id[n]) + ' ' + to_string(map[i][j][k+1].var_id[n]) + " 0"); 
                        //(¬b ∨ ¬c) ∧ (¬b ∨ ¬d) ∧ (¬b ∨ ¬e) ∧ (¬c ∨ ¬d) ∧ (¬c ∨ ¬e) ∧ (¬d ∨ ¬e)
                        ls.push_back(to_string(map[i][j+1][k].var_id[n]) + ' ' + to_string(map[i][j][k-1].var_id[n]) + " 0");
                        ls.push_back(to_string(map[i][j+1][k].var_id[n]) + ' ' + to_string(map[i][j-1][k].var_id[n]) + " 0");
                        ls.push_back(to_string(map[i][j+1][k].var_id[n]) + ' ' + to_string(map[i][j][k+1].var_id[n]) + " 0");
                        ls.push_back(to_string(map[i][j][k-1].var_id[n]) + ' ' + to_string(map[i][j-1][k].var_id[n]) + " 0");
                        ls.push_back(to_string(map[i][j][k-1].var_id[n]) + ' ' + to_string(map[i][j][k+1].var_id[n]) + " 0");
                        ls.push_back(to_string(map[i][j-1][k].var_id[n]) + ' ' + to_string(map[i][j][k+1].var_id[n]) + " 0");
                    }
                }
                    ...
                    //new ver
                    for(int n = 0; n < net_num; n++){
                        int a = map[i][j][k].var_id[n];
                        int b = map[i][j][k-1].var_id[n];
                        int c = map[i][j+1][k].var_id[n];
                        int d = map[i][j][k+1].var_id[n];
                        int e = map[i][j-1][k].var_id[n];
                        ls.push_back(to_string(-a)+' '+to_string(-b)+' '+to_string(-c)+' '+to_string(-d)+" 0");
                        ls.push_back(to_string(-a)+' '+to_string(-b)+' '+to_string(-c)+' '+to_string(-e)+" 0");
                        ls.push_back(to_string(-a)+' '+to_string(-b)+' '+to_string(-d)+' '+to_string(-e)+" 0");
                        ls.push_back(to_string(-a)+' '+to_string(b)+' '+to_string(c)+' '+to_string(d)+" 0");
                    ...
                    for(int n = 0; n < net_num; n++){
                        if(j == 0)
                            ls.push_back(to_string(map[i][j+1][k].var_id[n])+" 0");
                        else
                            ls.push_back(to_string(map[i][j-1][k].var_id[n])+" 0");
                    }
                    ...
                    */
                    ...
                    //new ver
                    for(int n = 0; n < net_num; n++){
                        ls.push_back(to_string(-map[i][j][k].var_id[n])+" 0");
                    }
                }
            }
        }
